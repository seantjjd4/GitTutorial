...
//			if (! _isReady) { // 2018.01.03 從下方搬上來, 無效
...
//			Framework.Game.resume(); // <-- 追看看會不會造成cycleCount的差異
		...
		var update = function () {
			if(_isReady) {  // <- 會不會跟這裡有關? Game.start().runFunction() (636) call Framework.Replay.setGameReady();
				_cycleCount++;
		...
		var resetWaitingCounter = function () {
			_waitingCounter = 0;
		};

		var getWaitingCounter = function () {
			return _waitingCounter;
		};
		...
		var waitFor = function (condition) {
			...
			item.replayFunction = function () {
				...
				_waitingCounter = 1;
//				_waitingCounter = 0;  // 2017.12.07 modify
//				_waitingCounter = -2;  // 2017.12.28 modify
				_isWaiting = true;
		...
		ReplayClass.prototype = {
			...
			resetWaitingCounter : resetWaitingCounter,
